        ...
        '>
            ...
            themes.map((theme) => (
              <MenubarItem key={theme.value} onClick={() => { }}>
                <Image src={theme.icon} alt={theme.value} width={16} height={16} className={`${mode === theme.value && 'active-theme'}`} />
                  <p className={`body-semibold text-light-500 ${mode === value ? 'text-primary-500' : 'text-dark100_light900'}`}>
                  {theme.label}
                </p>
              </MenubarItem>
            ))
...

          {
            themes.map(({ value, label, icon }) => (
              ...
              >
                <Image src={icon} alt={value} width={16} height={16} className={`${mode === value && 'active-theme'}`} loading='eager' />
                <p className={`body-semibold text-light-500 ${mode === value ? 'text-primary-500' : 'text-dark100_light900'}`}>
                  {label}
                </p>
              </MenubarItem>
